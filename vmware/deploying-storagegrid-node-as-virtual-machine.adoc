...
NOTE: 임시 설치 비밀번호는 노드 설치 중에만 사용됩니다. 노드가 그리드에 추가된 후에는 Recovery Package의  `Passwords.txt` 파일에 나열된 link:../admin/change-node-console-password.html["노드 콘솔 암호"]을(를) 사용하여 해당 노드에 액세스할 수 있습니다.
