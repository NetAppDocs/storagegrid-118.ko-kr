...
* 그리드 네트워크에서 그리드 노드 간 또는 StorageGRID 사이트 간에 NAT(네트워크 주소 변환)를 사용하지 않습니다. Grid Network에 전용 IPv4 주소를 사용하는 경우, 이러한 주소는 모든 사이트의 모든 그리드 노드에서 직접 라우팅할 수 있어야 합니다. NAT를 사용하여 공용 네트워크 세그먼트에서 그리드 네트워크를 브리징하는 것은 모두에게 투명한 터널링 애플리케이션을 사용하는 경우에만 지원됩니다
그리드의 노드입니다. 즉, 그리드 노드는 공용 IP 주소를 알 필요가 없습니다.
+
