...
복구 패키지 `.zip` 파일
 a| 
반드시 해야 합니다 link:downloading-recovery-package.html["최신 복구 패키지를 다운로드합니다"] `.zip` 파일 (`sgws-recovery-package-_id-revision_.zip`)를 클릭합니다. 장애가 발생할 경우 복구 패키지 파일을 사용하여 시스템을 복원할 수 있습니다.
