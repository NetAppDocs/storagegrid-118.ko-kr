...
** 기본 규칙을 준수하지 않으면 오류가 나타납니다. 규정 준수 규칙을 기본 규칙으로 포함하는 새 ILM 정책을 만들고 활성화해야 합니다. OK * 를 선택합니다. 그런 다음 새 정책을 생성하고 시뮬레이션한 다음 활성화합니다. 을 참조하십시오 link:creating-ilm-policy.html["ILM 정책을 생성합니다"] 를 참조하십시오.
