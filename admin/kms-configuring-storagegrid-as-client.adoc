...
NOTE: 이러한 지침은 Thales CipherTrust Manager 및 Hashicorp Vault에 적용됩니다. 지원되는 제품 및 버전 목록을 보려면 을 사용합니다 https://imt.netapp.com/matrix/#welcome["NetApp 상호 운용성 매트릭스 툴(IMT)"^].
...
** StorageGRID에서 키를 생성하도록 합니다. 테스트 후 저장하면 메시지가 표시됩니다 link:kms-adding.html#sg-create-key["클라이언트 인증서를 업로드하는 중입니다"].
